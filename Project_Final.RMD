...
Through our analysis, we will try to deduce the relationship between  ASD level, and car injuries. Precisely, we want to answer the following questions: **Are ASD equipped cars less likely to cause severe injuries than Level 2 equipped cars?**
...
```
### Combined
We combined the ADS equipped and Level 2 equipped data to create a combined dataset. We then created bar graphs to show the relationship between weather conditions and the number of accidents, and the relationship between injury severity and the number of accidents.
...
```
From these two graph, we can see that the number of accidents is higher in clear weather conditions compared to other weather conditions. The number of accidents is also higher in the "No Injuries Reported" category compared to other injury severity categories. These implies that for all the vehicles combined, accidents are more likely to occur in clear weather conditions and are more likely to result in no injuries.
...
We did the same analysis for ADS vehicles. The number of accidents is higher in clear weather conditions compared to other weather conditions. The number of accidents is also higher in the "No Injuries Reported" category compared to other injury severity categories. This implies that for ADS vehicles, accidents are more likely to occur in clear weather conditions and are more likely to result in no injuries.
...
We did the same analysis for Level 2 vehicles. The number of accidents is higher in other weather conditions compared to clear weather conditions. The number of accidents is also higher in the "No Injuries Reported" category compared to other injury severity categories. However, the number of the sum of accidents of the rest of the injury severity categories is higher compared to the "No Injuries Reported" category. This implies that for Level 2 vehicles, accidents are more likely to occur in other weather conditions and are more likely to result in injuries.
...
The bar graph shows the number of accidents that occurred in each injury severity group for ADS and Level2 vehicles. The severity of injuries is categorized into two groups: No Injuries/Minor and Moderate/Serious/Fatality.

We see that the percentage of accidents that cause severe injuries (Moderate/Serious/Fatality) is lower for ADS vehicles compared to Level 2 vehicles. More analysis is needed to determine if this difference is statistically significant.
...
$$\text{Test Statistic} = \hat{p}_1 - \hat{p}_2$$
```{r}
...
$$se = \sqrt{\frac{p(1-p)}{n_1} + \frac{p(1-p)}{n_2}} = 0.01809973$$

The standard error of the difference in sample proportions is 0.025. The z-score is -15.80717.

We then calculate the p-value using the z-score.

$$\text{p-value} = 2 \times P(Z < -|z|) = 2 \times P(Z < -15.80717) = 2.776592 \times 10^{-56}$$
...
The findings of this study indicate that ADS-equipped vehicles are less likely to cause severe injuries in car crashes compared to those equipped with Level 2 systems. This suggests that higher-level autonomous systems may offer better safety outcomes, likely due to their advanced capabilities in hazard detection and response.

These results could influence policy decisions aimed at promoting the adoption of more advanced ADS technologies to enhance road safety. By prioritizing the deployment of these systems, we can potentially reduce the severity of car accidents, alleviating the burden on healthcare systems and contributing to overall public safety.
